...
$grouptaskid = $mysqli->real_escape_string(strip_tags($_POST['grouptaskid']));
$taskid = $mysqli->real_escape_string(strip_tags($_POST['taskid']));
...
if ($return && $tablename=="projects" && $colname=="est_end_date") {
	if ( $stmt = $mysqli->prepare("UPDATE projects SET est_duration=DATEDIFF(est_end_date, start_date) WHERE id = ?")) {
